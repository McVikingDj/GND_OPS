// Gullknapp Ops Strip Board - v2.2
// Restored v2-style app (drag/drop + edit modal) but traffic-focused fields + training badges.

(function () {
  const STORAGE_KEY = "gullknapp_strips_v22";
  const SETTINGS_KEY = "gullknapp_settings_v22";
  ...
  const clearGroundBtn = document.getElementById("clearGroundBtn");
  ...
  function load() {
    ...
    state.settings = safeJsonParse(localStorage.getItem(SETTINGS_KEY), {}) || {};
    ...
    state.strips = state.strips
      ...
      .map(s => ({
        ...
        column: COLUMNS.includes(s.column) ? s.column : (s.status === "fueling" ? "ground" : s.status),
  ...
  function render() {
    ...
    for (const strip of state.strips) {
      ...
      const typeWake = `${escapeHtml(strip.aircraft || "—")}${strip.wake ? " / " + escapeHtml(strip.wake) : ""}`;
      ...
      el.innerHTML = `
        ...
        <div class="top">
          <div class="cell">
            <div class="miniLabel">TYPE/W</div>
    ...
    document.querySelectorAll(".strip-container").forEach(container => {
      new Sortable(container, {
        ...
        onEnd: function (evt) {
          ...
          // Update strip status based on new column, but preserve fueling if user wants it
          // If it was fueling and moved away from ground, drop fueling and set to that column
          if (newColumn !== "ground" && strip.status === "fueling") {
            strip.status = newColumn;
          } else if (newColumn === "ground" && strip.status !== "fueling") {
            strip.status = "ground";
          } else if (newColumn !== "ground") {
            strip.status = newColumn;
          }
  ...
  clearGroundBtn.addEventListener("click", () => {
    if (!confirm("Clear all strips in ON GROUND?")) return;
    state.strips = state.strips.filter(s => (s.status !== "ground" && s.status !== "fueling"));
    save();
    render();
  });
  ...
  stripForm.addEventListener("submit", (e) => {
    ...
    const next = {
      ...
      registration: String(f.registration.value || "").trim().toUpperCase(),
      wake: String(f.wake.value || "").trim().toUpperCase(),
    ...
    // Column derived from status (fueling stays in ground column)
    next.column = (next.status === "fueling") ? "ground" : next.status;
