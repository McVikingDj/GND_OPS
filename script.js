...
(function () {
  ...
  function applyAirTimerTransition(strip, prevStatus, nextStatus){
    ...
    // Start ONLY when entering AIRBORNE (not when entering PATTERN directly)
    if (!prevInAir && nextStatus === "airborne"){
  ...
  function render() {
    ...
    for (const strip of state.strips) {
      ...
      // Air time display
      let airMs = null;
      if ((strip.status === "airborne" || strip.status === "pattern") && typeof strip.airborneStartMs === "number" && strip.airborneStartMs){
